...
function drawBullets() { //총알생성
    ctx.fillStyle = "green";
    for (var i = 0; i < arrBullets.length; i++) {
        ctx.fillRect(arrBullets[i].x, arrBullets[i].y, 5, 4);
    }
}
function updateBullets() { //총알업뎃
    for (var i = 0; i < arrBullets.length; i++) {
        arrBullets[i].x += 4; 
    }
}
function checkBulletEnemyCollision(bullet, enemy) { //총알과 적 닿을시..
    return (
        bullet.x < enemy.x + enemy.wh &&
        bullet.x + 5 > enemy.x &&
        bullet.y < enemy.y + enemy.wh &&
        bullet.y + 4 > enemy.y
    );
}
function gameLoop() {
    ...
    for (var i = 0; i < arrBullets.length; i++) {
        for (var j = 0; j < arrEnemy_v16.length; j++) {
            if (checkBulletEnemyCollision(arrBullets[i], arrEnemy_v16[j])) {
                arrBullets.splice(i, 1);
                arrEnemy_v16.splice(j, 1);
                i--;
            }
        }
    }
